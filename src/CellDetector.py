...
class CellDetector(object):
    ...
    def __init__(self, config):
        ...
        self._area_threshold = None                     # Percentage of the circle that must be filleds
    ...
    def run(self, image):
        ...
        if all([self._kernel_size,
            ...
            for circle in circles:
                coverage = get_coverage(img, circle)
                if coverage > self._area_threshold:
                    cells.append(Cell(circle[0:2], circle[2]))
        else:
    ...
    def _configure(self, config):
        ...
        - pixels_per_micrometer not found.""")
            try:
                self._set_rbc_min_dist(config_dict["rbc_min_distance"][0])
            except KeyError:
                print("Warning: rbc_min_distance not found.")
            try:
                self._set_area_threshold(config_dict["area_threshold"][0])
            except KeyError:
    ...
    def _set_rbc_min_dist(self, dist):
        try:
            ...
            if self._min_rbc_dist <= 0:                         # If given value is <= 0
                if self._min_rbc_rad is not None:
...
def get_coverage(img, circle):
    ...
    for i in range(x - rad, x + rad + 1):                       # For each pixel in the x-direction
        for j in range(y - rad, y + rad + 1):                   # For each pixel in the y-direction
            in_image = 0 <= j < height and 0 <= i < width       # Bool describing if point is in the image
            in_circle = pythagoras((i - x), (j - y)) < rad      # Bool describing if point is in the circle
    ...
    percentage = black / float(black + non_black) * 100         # Percentage of pixels that are black
    return percentage
